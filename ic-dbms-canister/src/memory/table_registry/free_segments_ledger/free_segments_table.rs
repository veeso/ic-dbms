...
impl FreeSegmentsTable {
    ...
    pub fn load(page: Page) -> MemoryResult<Self> {
        let records = MEMORY_MANAGER.with_borrow(|mm| mm.read_at(page, 0))?;
        let max_records = Self::max_segments();
        Ok(Self {
            max_records,
            page,
            records,
    ...
    fn commit(&self) -> MemoryResult<()> {
        MEMORY_MANAGER.with_borrow_mut(|mm| mm.write_at(self.page, 0, &self.records))
    }
...
mod tests {
    ...
    fn test_should_encode_and_decode_free_segments_table() {
        ...
        table
            .insert_free_segment(1, 0, 50)
            .expect("insert_free_segment");
        table
            .insert_free_segment(2, 100, 75)
    ...
    fn test_should_insert_and_remove_free_segments() {
        ...
        table.insert_free_segment(1, 0, 100).expect("Insert failed");
        table
            .insert_free_segment(1, 100, 50)
    ...
    fn test_should_compute_max_segments() {
        ...
        let computed_max = FreeSegmentsTable::max_segments();
    ...
    fn test_should_panic_when_inserting_into_full_table() {
        let mut table = mock_table_with_size(1);

        table.insert_free_segment(1, 0, 100).expect("Insert failed");
        // This should panic
        table
            .insert_free_segment(1, 150, 50)
            .expect("Insert failed");
    }
    ...
    fn test_should_not_merge_non_adjacent_segments() {
        let mut table = mock_table();
        table.insert_free_segment(1, 0, 50).expect("Insert failed");
        table
            .insert_free_segment(1, 100, 50)
    ...
    fn test_should_tell_if_table_is_full() {
        let mut table = mock_table_with_size(2);
    ...
    fn test_should_commit_after_each_insert() {
        ...
        let reloaded_table = FreeSegmentsTable::load(table.page).expect("Reload failed");
    ...
    fn test_should_commit_after_remove() {
        ...
        let mut table = FreeSegmentsTable::load(table.page).expect("Reload failed");
        assert_eq!(table.records.0.len(), 1);
    ...
    fn mock_table() -> FreeSegmentsTable {
        mock_table_with_size(FreeSegmentsTable::max_segments())
    }

    fn mock_table_with_size(max_records: usize) -> FreeSegmentsTable {
        // create a page
        let page = MEMORY_MANAGER.with_borrow_mut(|mm| mm.allocate_page().expect("alloc failed"));
        FreeSegmentsTable {
