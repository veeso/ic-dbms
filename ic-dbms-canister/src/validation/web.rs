use ic_dbms_api::prelude::{IcDbmsError, Value};
...
impl Validate for MimeTypeValidator {
    fn validate(&self, value: &crate::prelude::Value) -> ic_dbms_api::prelude::IcDbmsResult<()> {
        let Value::Text(text) = value else {
            return Err(IcDbmsError::Validation("Value is not a Text".to_string()));
        };
        ...
        if parts.len() != 2 {
            return Err(IcDbmsError::Validation(format!(
...
mod tests {
    ...
    fn test_mime_type_validator() {
        ...
        for mime in valid_mime_types {
            let value = Value::Text(crate::prelude::Text(mime.to_string()));
            assert!(
    ...
    fn test_url_validator() {
        ...
        for url in valid_urls {
            let value = Value::Text(crate::prelude::Text(url.to_string()));
            assert!(
