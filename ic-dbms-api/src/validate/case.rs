...
impl Validate for SnakeCaseValidator {
    fn validate(&self, value: &Value) -> ic_dbms_api::prelude::IcDbmsResult<()> {
        ...
        let first_char = s.chars().next().ok_or_else(|| {
            crate::prelude::IcDbmsError::Validation(
                "Empty string is not valid snake_case".to_string(),
            )
        })?;
        if !first_char.is_lowercase() && first_char != '_' {
            return Err(crate::prelude::IcDbmsError::Validation(format!(
            ...
            .all(|c| c.is_lowercase() || c.is_ascii_digit() || c == '_')
        {
            ...
            Err(crate::prelude::IcDbmsError::Validation(format!(
                "Value '{s}' is not in snake_case format",
            )))
        }
    }
...
mod tests {
    ...
    fn test_snake_case_validator() {
        ...
        // Valid snake_case
        let value = Value::Text(ic_dbms_api::prelude::Text("valid_snake_case".to_string()));
        assert!(validator.validate(&value).is_ok());

        // Invalid snake_case (uppercase letter)
        let value = Value::Text(ic_dbms_api::prelude::Text("Invalid_Snake_Case".to_string()));
        assert!(validator.validate(&value).is_err());

        // Invalid snake_case (special character)
        let value = Value::Text(ic_dbms_api::prelude::Text(
        ...
        let value = Value::Text(ic_dbms_api::prelude::Text("_valid_snake_case".to_string()));
        assert!(validator.validate(&value).is_ok());

        // empty string
        let value = Value::Text(ic_dbms_api::prelude::Text("".to_string()));
        assert!(validator.validate(&value).is_err());
    ...
    fn test_kebab_case_validator() {
        ...
        // Valid kebab-case
        let value = Value::Text(ic_dbms_api::prelude::Text("valid-kebab-case".to_string()));
        assert!(validator.validate(&value).is_ok());

        // Invalid kebab-case (uppercase letter)
        let value = Value::Text(ic_dbms_api::prelude::Text("Invalid-Kebab-Case".to_string()));
        assert!(validator.validate(&value).is_err());
    ...
    fn test_camel_case_validator() {
        let validator = CamelCaseValidator;
        // Valid CamelCase
        let value = Value::Text(ic_dbms_api::prelude::Text("ValidCamelCase".to_string()));
        assert!(validator.validate(&value).is_ok());
        // Invalid CamelCase (starts with lowercase)
        let value = Value::Text(ic_dbms_api::prelude::Text("invalidCamelCase".to_string()));
        assert!(validator.validate(&value).is_err());
        // Invalid CamelCase (special character)
        let value = Value::Text(ic_dbms_api::prelude::Text("Invalid-CamelCase!".to_string()));
        assert!(validator.validate(&value).is_err());
        // Invalid CamelCase (contains underscore)
        let value = Value::Text(ic_dbms_api::prelude::Text("Invalid_CamelCase".to_string()));
        assert!(validator.validate(&value).is_err());
